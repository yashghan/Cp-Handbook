...
public:
    long long countQuadruplets(vector<int>& nums) {
        ...
        //let greaterThan[i][j] represent number of elements in the range [i..j] which are greater than nums[i]

        //let lesserThan[i][j] represent number of elements in the range [i..j] which are lesser than nums[j]
        for(int i = 0; i < n; i++){
